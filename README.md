...
- **Formula**: free-text expressions over traces and time, e.g. `sqrt({sine}^2 + {cosine}^2)` or `sin(2*pi*t)`. Traces are referenced as `{name}` (spaces/dots allowed); `t`, `pi`, `e` are built in. Supported: `+ - * / ^`, parentheses, implicit multiplication (`2{x}`, `{a}{b}`), and the functions `sqrt`, `root(x,n)`, `exp`, `sin`, `cos`, `tan`, `asin`, `acos`, `atan`, `atan2(y,x)`, `sinh`, `cosh`, `tanh`, `asinh`, `acosh`, `atanh`, `abs`, `ln`, `log10`, `log2`, `log(x)` (base-10), `log(x,b)` (base-b), `min(a,b,…)`/`max(a,b,…)` (per-sample extreme over any number of arguments), and `minh(a,b,…)`/`maxh(a,b,…)` (running extreme over the trace history, like the Min/Max operation — reset via the trace's ↻ button). The editor offers clickable chips for each trace (colored) plus `t`/`pi`/`e`/functions — `{trace}` references are highlighted in the trace's color — and a live LaTeX-style preview that renders parenthesized divisions as stacked fractions. `t` is in seconds and can be absolute (the sample's timestamp) or relative to a resettable origin ("Reset t" sets t = 0 at the newest sample).

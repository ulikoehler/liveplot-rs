    ...
    fn render_panel(&mut self, ui: &mut Ui, data: &mut ScopeData) {
        ...
        impl<'a> TableDelegate for TracesDelegate<'a> {
            fn header_cell_ui(&mut self, ui: &mut egui::Ui, cell: &egui_table::HeaderCellInfo) {
                ...
                let builder = egui::UiBuilder::new().max_rect(rect).layout(
            ...
            fn cell_ui(&mut self, ui: &mut egui::Ui, cell: &egui_table::CellInfo) {
                ...
                let r = &self.rows[row];
                let (rect, _resp) =
                    ui.allocate_exact_size(egui::vec2(self.col_w[col], 20.0), egui::Sense::click());
                let builder = egui::UiBuilder::new().max_rect(rect).layout(
                    egui::Layout::centered_and_justified(egui::Direction::LeftToRight),
                );
                let mut inner = ui.new_child(builder);
                match col {
                    ...
                    1 => {
                        ...
                        if resp.clicked() {
                            if !r.is_free {
                                let cur = self.look_editor_trace.clone();
                                *self.look_editor_trace = if cur.as_deref() == Some(&r.name) {
                                    None
                                } else {
                                    Some(r.name.clone())
                                };
                    ...
                    2 => {
                        let mut sel = self.data.hover_trace.clone();
                        let is_selected = (r.is_free && sel.is_none())
                            || (!r.is_free && sel.as_ref() == Some(&r.name));
                        let resp = inner
                            .selectable_label(is_selected, if r.is_free { "Free" } else { "Use" });
                        if resp.clicked() {
                            sel = if r.is_free {
                                None
                            } else {
                    ...
                    3 => {
                        if r.is_free {
                            ...
                            if inner.checkbox(&mut vis, "").changed() {
                                tr.look.visible = vis;
                            }
                        }
                    }
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| {
                ...
                let mut delegate = TracesDelegate {
                    ...
                    rows: rows.clone(),
                    col_w: w,
                    look_editor_trace: &mut self.look_editor_trace,
                };
                Table::new()
